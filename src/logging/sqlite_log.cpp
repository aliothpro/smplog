    ...
    /*  考察下是否需要分表  */
    {
		...
		if (create)
		{
			if (nullptr != m_Database)
			{
				...
				sprintf_std_string(createSQL, "CREATE TABLE %s (id INTEGER PRIMARY KEY, level TEXT, ts TEXT, msg BLOB, module TEXT)", m_TableName.c_str());
                try{
        ...
        try{
            ...
            for (auto &line : logCache)
            {
                ...
                {
                    sprintf_std_string(insertSQL, "INSERT INTO %s (level,ts,msg,module) VALUES (\"%s\", \"%s\", \"%s\", \"%s\")",
                ...
                {
                    sprintf_std_string(insertSQL, "INSERT INTO %s (level,ts,msg,module)  VALUES (\"%s\", \"%s\", ?, \"%s\")",
